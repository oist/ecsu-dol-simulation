...
class SimulationSynergy:
    ...
    synergy_padding = 100 # adding this to synergy to avoid negative numbers
    ...
    def load_from_file(file_path, **kwargs):
        ...
        sim = SimulationSynergy(**obj_dict)
    ...
    def init_synergy_data(self):
        self.synergy_data = {
            'agents_brain_input': np.zeros((self.num_trials, self.num_agents, self.num_data_points, self.num_neurons)),
            'agents_brain_state': np.zeros((self.num_trials, self.num_agents, self.num_data_points, self.num_neurons)),
            'agents_brain_output': np.zeros((self.num_trials, self.num_agents, self.num_data_points, self.num_neurons)),
            'delta_tracker_target': np.zeros((self.num_trials, self.num_data_points))
        }

    def save_synergy_data_step(self, t, i):
        for a, o in enumerate(self.agents):
            if a == self.ghost_index:
                continue
            self.synergy_data['agents_brain_input'][t][a][i] = o.brain.input
            self.synergy_data['agents_brain_state'][t][a][i] = o.brain.states
            self.synergy_data['agents_brain_output'][t][a][i] = o.brain.output
            self.synergy_data['delta_tracker_target'][t][i] = self.tracker.delta_target
    ...
    def prepare_trial(self, t):
        ...
        # self.save_synergy_data_step(t, 0)
    ...
    def compute_synergy_trials(self):

        synergy_trials = []

        cond_multi_mi_calc = jp_kraskov_pkg.ConditionalMutualInfoCalculatorMultiVariateKraskov1()
        cond_multi_mi_calc.setProperty("NOISE_LEVEL_TO_ADD", "0") # no noise for reproducibility        
        ...
        for t in range(self.num_trials):
            ...
            cond_multi_mi_calc.initialise(agent1_data.shape[1], agent2_data.shape[1], 1)		

            cond_multi_mi_calc.setObservations(
                jp.JArray(jp.JDouble, 2)(agent1_data), 
                jp.JArray(jp.JDouble, 2)(agent2_data), 
                jp.JArray(jp.JDouble, 2)(target_pos)
            )

            cond_multivar_mi = cond_multi_mi_calc.computeAverageLocalOfObservations()

            multivar_mi_calc.initialise(agent1_data.shape[1], agent2_data.shape[1])		
            ...
            synergy = cond_multivar_mi - multivar_mi + self.synergy_padding
            synergy_trials.append(synergy)
        return synergy_trials
        ...
        for self.sim_index in range(num_simulations):
            ...
            for t in range(self.num_trials):
                ...
                for i in range(self.num_data_points): 
                    ...
                    self.save_synergy_data_step(t, i)
            ...
            synergy_trials = self.compute_synergy_trials()
            exp_perf = np.mean(synergy_trials)
            ...
            if self.data_record:
                self.data_record.update({
                    ...
                    'trials_performances': synergy_trials,
    ...
    def evaluate(self, populations, random_seed):
        ...
        sim_copy = SimulationSynergy(**asdict(self))
    ...
    def normalize_performance(self, performance):
        ...
        '''
        return performance - self.synergy_padding
